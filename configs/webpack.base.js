...
let config = {
	...
	plugins: [
		...
		// 如需另外新增资源文件夹，修改这里  data 换成自己对应需要的即可
...
//获取所有html页面
let tplPages = Object.keys(getEntry('./src/*.art'));
tplPages.forEach((pathname) => {
	let data = require(`../data/${pathname}.js`);

	// TODO: 测试不同文件插入不同的cdn
	data.forEach((v) => {
		paramConfig.side.forEach((side) => {
			let type = side == 'pc' ? '' : `/${side}`
			let conf = {
				chunksSortMode: 'manual',
				filename: `${v.lang + type}/${paramConfig.year}/${paramConfig.ztName}/${pathname}.html`, //生成的html存放路径，相对于path
				template: './src/' + pathname + '.art', //ejs模板路径,前面最好加上loader用于处理
				chunks: ['vendor', pathname],
				templateParameters: { myInfo: v }
			};
